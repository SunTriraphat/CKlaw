{{-- @foreach ($data as $key => $dataProperty)
    <div class="card border border-white mb-2 p-2">
        <div class="card-title">
            <span class=" fw-semibold text-primary"><i class="fa-solid fa-address-book"></i> ขั้นตอนฟ้อง (45-60
                วัน)</span>
            <a data-link="{{ route('Law.show', $dataProperty->id) }}?type={{ 'Editcus2' }}" data-bs-toggle="modal"
            ...
            <div class="row">
                <div class="col-6">
                    <table class="table table-sm">
                        <tr>
                            ...
                            @if (@$dataProperty->date_witness != null)
                                <td class="text-end">{{ formatDateThai(@$dataProperty->date_witness) }}</td>
                            @else
                        ...
                        <tr>
                            ...
                            @if (@$dataProperty->date_postponed != null)
                                <td class="text-end">{{ formatDateThai(@$dataProperty->date_postponed) }}</td>
                            @else
                                <td class="text-end">-</td>
                            @endif
                        </tr>
                ...
                <div class="col-6">
                    <table class="table table-sm">
                        <tr>
                            ...
                            <td class="text-end">{{ @$dataProperty->witness_status }}</td>
                        </tr>
                ...
                <div class="col-12">
                    <tr>
                        ...
                        <td class="text-end">{{ @$dataProperty->witness_note }} </td>
                    </tr>
                </div>
...
<div class="card border border-white mb-2 p-2">
    <div class="card-title">
        <span class=" fw-semibold text-primary"><i class="fa-solid fa-address-book"></i> ค่าใช้จ่าย </span>
             ...
             <a data-link="{{ route('Fin.show', $dataProperty->cus_id) }}?type={{ 'EditFinanceWitness' }}" data-bs-toggle="modal"
            data-bs-target="#modal-xl" type="button" class="btn-sm float-end btn btn-light rounded-circle">
            <i class="fa-solid fa-pen-to-square"></i>
...
{{-- @php
    ...
    <div class="card border border-white mb-2 p-2">
        <div class="card-title">
            ...
            <a data-link="{{ route('Law.show', @$data->cus_id) }}?type={{ 'Editcus6' }}" data-bs-toggle="modal"
                data-bs-target="#modal-xl" type="button" class="btn-sm float-end btn btn-light rounded-circle">
                <i class="fa-solid fa-pen-to-square"></i>
            </a>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    <table class="table table-sm">
                        <tr>
                            <th class="text-start">วันที่ออกหมายตั้งเจ้าพนง.บังคับคดี</th>
                            @if (@$data->date_app != null)
                                <td class="text-end">{{ formatDateThai(@$data->date_app) }}</td>
                            @else
                                <td class="text-end">-</td>
                            @endif
                        </tr>
                ...
                <div class="col-12">
                    <tr>
                        <th class="text-start"><strong>หมายเหตุ : </strong></th>
                        <td class="text-end">{{ @$data->app_note }} </td>
                    </tr>
                </div>
            </div>
    ...
    <a data-link="{{ route('Fin.show',$data->cus_id) }}?type={{ 'InsertFinanceProperty' }}" data-bs-toggle="modal"
...
@foreach ($dataFinance as $finance)
    ...
    @if ($finance->levels == 'ขั้นตั้งเจ้าพนักงาน'&& ($finance->status == NULL || $finance->status != 'ยกเลิก'))
        <div class="card border border-white mb-2 p-2">
            <div class="card-title">
                ...
                <a data-link="{{ route('Fin.show', $finance->id) }}?type={{ 'EditFinanceProperty' }}"
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-6">
                        <table class="table table-sm">
                            <tr>
                                <th class="text-start">ค่าตรวจกรรมสิทธิ์ทรัพย์</th>
                                {{-- <td class="text-end">{{ date_format(date_create(@$dataProperty->date_tribunal), 'd/m/Y') }}</td> --}}
                                <td class="text-end">{{ number_format(@$finance->check_ownership, 2) }}</td>
                            </tr>
                        </table>
                    </div>
                    <div class="col-6">
                        <table class="table table-sm">
                            <tr>
                                <th class="text-start">ค่าคัดถ่ายและรับรองเอกสาร</th>
                                <td class="text-end">{{ number_format(@$finance->copy_documents2, 2) }}</td>
                            </tr>
                    ...
                    <div class="col-6">
                        <table class="table table-sm">
                            <tr>
                                <th class="text-start">ค่านำชี้ทรัพย์</th>
                                <td class="text-end">{{ number_format(@$finance->point_property, 2) }}</td>
