    ...
    <div class="modal-body">
        ...
        <form id='edit_cus2' enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="mb-3 ">
                        ...
                        <input type="date" lang="th-th" value="{{ trim(@$data->sub_date) }}" name="sub_date"
                            id="sub_date" class="form-control">
                    </div>


                    <div>
                        <input type="checkbox" value="Y" id="status" name="status"
                            {{ $data->status != 'ขั้นส่งคำบังคับ' ? 'checked disabled' : '' }} />
            ...
            <div class="row">
                <div class="mb-3 input-bx">
                    <span>หมายเหตุ</span>
                    <textarea class="form-control" name="command_note" id="command_note" required>{{ trim(@$data->command_note) }}</textarea>
                </div>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary " id="saveEditBtn">
                    <span class="spinner-border spinner-border-sm" role="status" id="loading-cus3" aria-hidden="true"
                        style="display: none"></span>
                    บันทึก</button>
                <button type="button" class="btn btn-danger " id="close-cus3" class="close" data-bs-dismiss="modal"
...
<script type="text/javascript">
    $(function() {
        ...
        $('#saveEditBtn').click(function(e) {
            ...
            let witness_status = $('#witness_status').val();
            let link = "{{ route('Law.update', 'id') }}?type={{ 'updateDataCus3' }}";
            ...
            if (date_witness != '' && witness_status != '') {
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        $('#close-cus3').prop('disabled', false);
                        $('#loading-cus3').hide();
                        Swal.fire({
                            icon: 'success',
                            title: `SUCCESS `,
                        ...
                        if (result.message == 'อัพเดตสถานะ') {
                            ...
                            $('#content-sendCommand').html(result.html)
                        }
                    },
